...
{\scriptsize
\begin{algorithm}[H]
    \caption{Quickselect $\mathcal{O}(n^2)$}
    \label{FWAlgorithm}
    \SetAlgoLined
    \SetKwInOut{Input}{Input}
    \SetKwInOut{Output}{Output}
...
{\scriptsize
\begin{algorithm}[H]
    \caption{Quicksort $\mathcal{O}(n\cdot \log \cdot n)$}
    \label{FWAlgorithm}
    \SetAlgoLined
    \SetKwInOut{Input}{Input}
    \SetKwInOut{Output}{Output}
...
Runtime: $\mathcal{O}(n\cdot \log \cdot n)$, worst case $\Theta(n^2)$ if worst pivots are selected each time.
...
\begin{itemize}
	...
	\begin{itemize}
	    ...
	    \item Johnsons in a sparse graph. $\mathcal{O}(|V|^{2}\log |V|+|V||E|)$
	\end{itemize} 
